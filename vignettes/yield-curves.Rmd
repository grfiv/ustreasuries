...
```{r YC_now}
last_two_years <- dplyr::filter(all_data, 
                                    all_data$NEW_DATE>=Sys.Date()-365*2 &
                                    all_data$NEW_DATE<=Sys.Date())

nrows <- nrow(last_two_years)
bits <- as.integer(nrows/5)

PrintYieldCurves(last_two_years, 
                 rows=c(1, bits, 2*bits, 3*bits, 4*bits, nrows), 
                 title="Yield Curves For The Last Two Years")
```
